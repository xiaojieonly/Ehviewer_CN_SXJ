 ...
 * legacy/unknown strategy fallback, §3.8 platform edges), plus the a2-line
 * {@code aliveRecordWins} §3.8 double-alive guard unit matrix.
 *
 * <p>Rows (client view):
 * <ol>
...
public class WebUiSyncClientMatrixTest {
    ...
    /**
     * Unit matrix for the client-side §3.8 double-alive guard (a2-line
     * additive kept by the W4 port): a pulled alive record colliding with a
     * locally-alive record is arbitrated exactly like the server does — A/C
     * cross-platform conflicts are won unconditionally by the priority
     * platform (timestamp does not arbitrate, §1.4); same-platform conflicts
     * and the B strategy fall back to last-write-wins.
     */
    @Test
