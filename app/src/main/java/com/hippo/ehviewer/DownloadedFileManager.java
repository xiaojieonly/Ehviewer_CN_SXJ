...
public class DownloadedFileManager {
    ...
    public DownloadedFile getFileByToken(String token) {
        ...
        try {
            ...
            if (file == null) {
                ...
                Log.d(TAG, "Found file for token: " + token + ", filename: " + file.getFilename());
            }
            return file;
        } catch (Exception e) {
    ...
    public boolean addOrUpdateFile(String token, long gid, String filename, String path, long size) {
        ...
        Log.d(TAG, "addOrUpdateFile called - token: " + token + ", gid: " + gid + ", filename: " + filename + ", size: " + size);
        
        try {
            DownloadedFile file = getFileByToken(token);
            if (file == null) {
                Log.d(TAG, "Creating new file record for token: " + token);
                // 新建文件记录
                ...
                Log.d(TAG, "Successfully inserted file record for token: " + token);
            } else {
                Log.d(TAG, "Updating existing file record for token: " + token);
                // 更新现有记录
                file.setLast_accessed(System.currentTimeMillis());
                if (size > 0) {
                    file.setSize(size);
                ...
                Log.d(TAG, "Successfully updated file record for token: " + token);
            }
            return true;
        } catch (Exception e) {
     ...
     */
    public void scanDownloadDirectories(@Nullable ScanProgressListener progressListener) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                // 扫描所有画廊目录
                UniFile[] galleryDirs = downloadDir.listFiles();
                if (galleryDirs == null) {
                    galleryDirs = new UniFile[0];
                }
                
                mScanTotal.set(galleryDirs.length);
                
                Log.i(TAG, "Found " + galleryDirs.length + " gallery directories to scan");
                
                for (int i = 0; i < galleryDirs.length; i++) {
                    UniFile galleryDir = galleryDirs[i];
                    if (!galleryDir.isDirectory()) {
                        Log.d(TAG, "Skipping non-directory: " + galleryDir.getName());
                        continue;
                    }
                    
                    Log.d(TAG, "Scanning gallery directory [" + (i+1) + "/" + galleryDirs.length + "]: " + galleryDir.getName());
                    
                    try {
                        scanGalleryDirectory(galleryDir);
                        Log.d(TAG, "Successfully scanned gallery directory: " + galleryDir.getName());
                    } catch (Exception e) {
                        Log.e(TAG, "Error scanning gallery directory: " + galleryDir.getName(), e);
                    }
                    
                    mScanProgress.set(i + 1);
                    
                    if (progressListener != null) {
                        progressListener.onProgress(i + 1, galleryDirs.length);
                    }
                }
    ...
    private void scanGalleryDirectory(UniFile galleryDir) throws Exception {
        ...
        // 读取SpiderInfo
        SpiderInfo spiderInfo = SpiderInfo.read(ehviewerFile);
        if (spiderInfo == null) {
        ...
        Log.d(TAG, "Found " + imageFiles.length + " files in directory");
        ...
        for (UniFile file : imageFiles) {
            ...
            // 从文件名提取token（假设格式为 index-token.ext）
            ...
            // 如果token是纯数字（可能是索引），则与画廊GID组合以确保全局唯一性
            String token;
            if (rawToken.matches("\\d+")) {
                // 纯数字，与GID组合
                token = spiderInfo.gid + "_" + rawToken;
                Log.d(TAG, "Raw token is numeric, generating composite token: " + token);
            } else {
                token = rawToken;
            }
            
            Log.d(TAG, "Final token: " + token + " from filename: " + filename + ", raw token: " + rawToken);
            ...
            try {
                addOrUpdateFile(token, spiderInfo.gid, filename, path, size);
