...
class LimitsCountView : FrameLayout {
    ...
    private fun resetLimit(view: View) {
        ...
        val callback: SiteClient.Callback<HomeDetail> = LimitsCountDataListener(context, view)

        val request = SiteRequest()
            .setMethod(SiteClient.METHOD_RESET_LIMIT)
    ...
    private fun onLoadData(view: View) {
        refreshIcon!!.visibility = GONE
        refreshing!!.visibility = VISIBLE
        val callback: SiteClient.Callback<HomeDetail>
        if (view === this) {
        ...
        val request = SiteRequest()
            .setMethod(SiteClient.METHOD_GET_HOME)
            .setCallback(callback)
        SiteApplication.getSiteClient(context).execute(request)
    }
...
//            fromGallery.setVisibility(GONE);
    ...
    protected inner class LimitsCountDataListener : SiteClient.Callback<HomeDetail> {
        private val context: Context
