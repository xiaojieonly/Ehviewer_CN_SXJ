...
import com.hippo.anotherviewer.util.MiuiOptimizationHelper
...
class DownloadService : Service(), DownloadManager.DownloadListener {
    ...
    override fun onCreate() {
        ...
        mDownloadManager = SiteApplication.getDownloadManager(applicationContext)
    ...
    override fun onFinish(info: DownloadInfo) {
        ...
        if (index < 0) { // Not contain
            ...
            sItemTitleArray.put(gid, SiteUtils.getSuitableTitle(info))
            sDownloadedCount++
            if (finish) {
                sFinishedCount++
            } else {
    ...
    private fun initWakeLock() {
        try {
            ...
            mWakeLock = powerManager.newWakeLock(
                ...
                "AnotherViewer:DownloadWakeLock"
            ).apply {
                // 设置为可计数，避免重复释放导致崩溃
