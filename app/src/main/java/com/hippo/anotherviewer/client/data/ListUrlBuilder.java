...
public class ListUrlBuilder implements Cloneable, Parcelable {
    ...
    private int mCategory = SiteUtils.NONE;
    ...
    public void setQuery(String query) {
        ...
        for (String str : querys) {
            ...
            switch (key) {
                case "f_cats":
                    int cats = NumberUtils.parseIntSafely(value, SiteConfig.ALL_CATEGORY);
                    category |= (~cats) & SiteConfig.ALL_CATEGORY;
                    break;
                case "f_doujinshi":
                    if ("1".equals(value)) {
                        category |= SiteConfig.DOUJINSHI;
                    }
                    break;
                case "f_manga":
                    if ("1".equals(value)) {
                        category |= SiteConfig.MANGA;
                    }
                    break;
                case "f_artistcg":
                    if ("1".equals(value)) {
                        category |= SiteConfig.ARTIST_CG;
                    }
                    break;
                case "f_gamecg":
                    if ("1".equals(value)) {
                        category |= SiteConfig.GAME_CG;
                    }
                    break;
                case "f_western":
                    if ("1".equals(value)) {
                        category |= SiteConfig.WESTERN;
                    }
                    break;
                case "f_non-h":
                    if ("1".equals(value)) {
                        category |= SiteConfig.NON_H;
                    }
                    break;
                case "f_imageset":
                    if ("1".equals(value)) {
                        category |= SiteConfig.IMAGE_SET;
                    }
                    break;
                case "f_cosplay":
                    if ("1".equals(value)) {
                        category |= SiteConfig.COSPLAY;
                    }
                    break;
                case "f_asianporn":
                    if ("1".equals(value)) {
                        category |= SiteConfig.ASIAN_PORN;
                    }
                    break;
                case "f_misc":
                    if ("1".equals(value)) {
                        category |= SiteConfig.MISC;
                    }
                    break;
                case "f_search":
                    try {
        ...
        switch (mMode) {
            ...
            case MODE_SUBSCRIPTION: {
                ...
                if (mMode == MODE_NORMAL) {
                    ...
                    url = SiteUrl.getWatchedUrl();
                }

                UrlBuilder ub = new UrlBuilder(url);
                if (mCategory != SiteUtils.NONE) {
                    ub.addQuery("f_cats", (~mCategory) & SiteConfig.ALL_CATEGORY);
                }
                // Search key
                if (mKeyword != null) {
            ...
            case MODE_UPLOADER: {
                StringBuilder sb = new StringBuilder(SiteUrl.getHost());
                sb.append("uploader/");
                try {
            ...
            case MODE_WHATS_HOT:
                return SiteUrl.getPopularUrl();
            case MODE_IMAGE_SEARCH:
                return SiteUrl.getImageSearchUrl();
            case MODE_TOP_LIST:
                StringBuilder sb = new StringBuilder(SiteUrl.getTopListUrl());
                sb.append("?");
                sb.append(mFollow);
                if (mPageIndex==0){
                    return sb.toString();
