...
public class SiteCookieStore extends CookieRepository {
    ...
    public static final Cookie sTipsCookie =
            new Cookie.Builder()
                    .name(SiteConfig.KEY_CONTENT_WARNING)
                    .value(SiteConfig.CONTENT_WARNING_NOT_SHOW)
                    .domain(SiteUrl.DOMAIN_E)
                    .path("/")
                    .expiresAt(Long.MAX_VALUE)
                    .build();

    public SiteCookieStore(Context context) {
    ...
    public boolean hasSignedIn() {
        HttpUrl url = HttpUrl.parse(SiteUrl.HOST_E);
    ...
    public List<Cookie> loadForRequest(HttpUrl url) {
        ...
        boolean checkTips = domainMatch(url, SiteUrl.DOMAIN_E);
        ...
        if (checkTips) {
            ...
            for (Cookie cookie: cookies) {
                ...
                if (SiteConfig.KEY_CONTENT_WARNING.equals(name)) {
                    continue;
                }
                if (SiteConfig.KEY_UCONFIG.equals(name)) {
                    continue;
                }
                result.add(cookie);
            }
