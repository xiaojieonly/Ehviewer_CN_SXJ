...
import com.hippo.anotherviewer.client.exception.OffensiveException;
import com.hippo.anotherviewer.client.exception.PiningException;
...
public class TopListParser {
    ...
    public static SiteTopListDetail parse(String body) throws SiteException {
        ...
        if (m.find()) {
            throw new SiteException(m.group(1));
        }

        SiteTopListDetail ehTopListDetail = new SiteTopListDetail();
        ...
        ehTopListDetail.galleryTopListInfo = parseInfo(elements.get(1), SiteTopListDetail.ListType.GALLERY);
        ehTopListDetail.uploaderTopListInfo = parseInfo(elements.get(3), SiteTopListDetail.ListType.UPLOADER);
        ehTopListDetail.taggingTopListInfo = parseInfo(elements.get(5), SiteTopListDetail.ListType.TAGGING);
        ehTopListDetail.hentaiHomeTopListInfo = parseInfo(elements.get(7), SiteTopListDetail.ListType.HENTAI_HOME);
        ehTopListDetail.ehTrackerTopListInfo = parseInfo(elements.get(9), SiteTopListDetail.ListType.EH_TRACKER);
        ehTopListDetail.cleanUpTopListInfo = parseInfo(elements.get(11), SiteTopListDetail.ListType.CLEANUP);
        ehTopListDetail.ratingAndReviewingTopListInfo = parseInfo(elements.get(13), SiteTopListDetail.ListType.RATING_AND_REVIEWING);


        return ehTopListDetail;
    }

    private static TopListInfo parseInfo(Element element, SiteTopListDetail.ListType type) {
