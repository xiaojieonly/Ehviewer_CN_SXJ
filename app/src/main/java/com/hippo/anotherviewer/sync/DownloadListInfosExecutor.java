...
public class DownloadListInfosExecutor {
    ...
    private List<DownloadInfo> filterDownloadKind(int state) {
        ...
        if (kind == SiteUtils.ALL_CATEGORY) {
            return mList;
        }
    ...
    protected List<DownloadInfo> searchingInBackground() {
        ...
        for (int i = 0; i < mList.size(); i++) {
            DownloadInfo info = mList.get(i);
            if (SiteUtils.judgeSuitableTitle(info, mSearchKey)) {
                cache.add(info);
    ...
    private ArrayList<String> searchTagList(long gid) {
        GalleryTags tags = SiteDB.queryGalleryTags(gid);
    ...
    private int kindValue(int id) {
        return switch (id) {
            case R.id.doujinshi -> SiteConfig.DOUJINSHI;
            case R.id.manga -> SiteConfig.MANGA;
            case R.id.artist_cg -> SiteConfig.ARTIST_CG;
            case R.id.game_cg -> SiteConfig.GAME_CG;
            case R.id.western -> SiteConfig.WESTERN;
            case R.id.non_h -> SiteConfig.NON_H;
            case R.id.image_set -> SiteConfig.IMAGE_SET;
            case R.id.cosplay -> SiteConfig.COSPLAY;
            case R.id.asian_porn -> SiteConfig.ASIAN_PORN;
            case R.id.misc -> SiteConfig.MISC;
            default -> SiteUtils.ALL_CATEGORY;
        };
    }
