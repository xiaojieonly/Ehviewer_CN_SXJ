...
public class BookmarksDraw {
    ...
    public BookmarksDraw(@NonNull Context context, LayoutInflater inflater, SiteTagDatabase ehTags) {
    ...
    public View onCreate(GalleryListScene scene) {
        ...
        List<QuickSearch> quickSearchList = SiteDB.getAllQuickSearch();
        //汉化标签
        ...
        if (judge && !quickSearchList.isEmpty()) {
            for (int i = 0; i < quickSearchList.size(); i++) {
                ...
                if (name != null && 2 == name.split(":").length) {
                    ...
                    SiteDB.updateQuickSearch(quickSearchList.get(i));
                }
            }
