...
public final class CommonOperations {
    ...
    private static void doAddToFavorites(Activity activity, GalleryInfo galleryInfo,
                                         int slot, SiteClient.Callback<Void> listener) {
        if (slot == -1) {
            SiteDB.putLocalFavorite(galleryInfo);
            listener.onSuccess(null);
        } else if (slot >= 0 && slot <= 9) {
            SiteClient client = SiteApplication.getSiteClient(activity);
            SiteRequest request = new SiteRequest();
            request.setMethod(SiteClient.METHOD_ADD_FAVORITES);
    ...
    public static void addToFavorites(final Activity activity, final GalleryInfo galleryInfo,
                                      final SiteClient.Callback<Void> listener, boolean isDefaultFavSolt) {
    ...
    public static void removeFromFavorites(Activity activity, GalleryInfo galleryInfo,
                                           final SiteClient.Callback<Void> listener) {
        SiteDB.removeLocalFavorites(galleryInfo.gid);
        SiteClient client = SiteApplication.getSiteClient(activity);
        SiteRequest request = new SiteRequest();
        request.setMethod(SiteClient.METHOD_ADD_FAVORITES);
    ...
    private static class DelegateFavoriteCallback implements SiteClient.Callback<Void> {

        private final SiteClient.Callback<Void> delegate;
        private final GalleryInfo info;
        private final String newFavoriteName;
        private final int slot;

        DelegateFavoriteCallback(SiteClient.Callback<Void> delegate, GalleryInfo info,
        ...
        public void onSuccess(Void result) {
            ...
            SiteApplication.getFavouriteStatusRouter().modifyFavourites(info.gid, slot);
        }
    ...
    public static void startDownload(final MainActivity activity, final List<GalleryInfo> galleryInfos, boolean forceDefault) {
        final DownloadManager dm = SiteApplication.getDownloadManager(activity);
