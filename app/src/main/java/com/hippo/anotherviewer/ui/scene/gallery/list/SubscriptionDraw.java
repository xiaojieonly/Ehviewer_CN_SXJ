...
import static com.hippo.anotherviewer.Settings.*;
...
public class SubscriptionDraw {
    ...
    final private SiteApplication ehApplication;

    private ListView listView;
    private ProgressView progressView;
    private FrameLayout frameLayout;
    private TextView textView;
    private final SiteClient ehClient;
    ...
    public SubscriptionDraw(@NonNull Context context, LayoutInflater inflater, SiteClient ehClient, String mTag, SiteTagDatabase ehTags) {
        ...
        ehApplication = (SiteApplication) context.getApplicationContext();
    }
    ...
    private void seeDetailPage() {
        ...
        SiteApplication.saveUserTagList(context, userTagList);
...
//        }
    ...
    private void loadData() throws SiteException {
        boolean requested = request();
        if (!requested) {
            throw new SiteException("请求数据失败请更换IP地址或检查网络设置是否正确~");
        }
    }
    ...
    private void requestTag(String tagName, boolean tagState) {
        ...
        SiteClient.Callback<UserTagList> callback = new SubscriptionDetailListener(context, activity.getStageId(), mTag);
...
//        String url = SiteUrl.getTopListUrl();
        String url = SiteUrl.getMyTag();
        ...
        SiteRequest mRequest = new SiteRequest()
                .setMethod(SiteClient.METHOD_GET_WATCHED)
