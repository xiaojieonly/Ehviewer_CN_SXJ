...
import static com.hippo.anotherviewer.spider.SpiderQueen.SPIDER_INFO_FILENAME;
...
public class SpiderInfo {
    ...
    public synchronized void writeNewSpiderInfoToLocal(@NonNull SpiderDen spiderDen, Context context) {
        ...
        if (downloadDir != null) {
            ...
            // Read from cache
            OutputStreamPipe pipe = SiteApplication.getSpiderInfoCache(context).getOutputStreamPipe(Long.toString(gid));
            try {
                pipe.obtain();
                write(pipe.open());
            } catch (IOException e) {
