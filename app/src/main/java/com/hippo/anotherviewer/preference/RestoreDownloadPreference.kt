...
class RestoreDownloadPreference : Preference {
    ...
    private inner class RestoreTask(private val mContext: Context) {
        private val mApplication: SiteApplication =
            mContext.applicationContext as SiteApplication
        private val mManager: DownloadManager =
            SiteApplication.getDownloadManager(mApplication)
        private val mHttpClient: OkHttpClient =
            SiteApplication.getOkHttpClient(mApplication)
        ...
        private fun doInBackground(): Any? {
            ...
            return try {
                SiteEngine.fillGalleryListByApi(
                    null,
                    mHttpClient,
                ...
                if (list.isEmpty()) {
                    ...
                    while (i < n) {
                        ...
                        if (null != item.title) {
                            ...
                            SiteDB.putDownloadDirname(item.gid, item.dirname)
                            count++
                        }
                        i++
                    }
