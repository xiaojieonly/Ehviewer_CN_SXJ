...
public final class SmbConnection {
    ...
    public boolean exists(String path) throws IOException {
        ...
        try {
            String smbPath = normalizeSmbPath(path);
            return TextUtil.isEmpty(smbPath) || share.fileExists(smbPath) || share.folderExists(smbPath);
        } finally {
            releaseShare(share);
        }
    }
    ...
    public boolean isDirectory(String path) throws IOException {
        if (TextUtil.isEmpty(normalizeSmbPath(path))) {
            return true;
        }
        DiskShare share = getShare();
        try {
    ...
    public boolean ensureDirectory(String path) throws IOException {
        ...
        try {
            String smbPath = normalizeSmbPath(path);
            if (TextUtil.isEmpty(smbPath)) {
                return true;
            }
            String[] parts = smbPath.split("/");
            String current = "";
            for (String part : parts) {
                if (TextUtil.isEmpty(part) || ".".equals(part) || "..".equals(part)) {
                    continue;
                }
                current = TextUtil.isEmpty(current) ? part : current + "/" + part;
    ...
    public void writeFile(String path, java.io.InputStream source, java.lang.Runnable onChunk) throws IOException {
    ...
    public void writeFile(String path, java.io.InputStream source, boolean append,
            java.lang.Runnable onChunk) throws IOException {
        if (append) {
    ...
    private List<SmbEntry> listEntries(DiskShare share, String path) {
        try {
            List<FileIdBothDirectoryInformation> entries = share.list(TextUtil.isEmpty(path) ? "" : path);
            List<SmbEntry> results = new ArrayList<>(entries.size());
            for (FileIdBothDirectoryInformation entry : entries) {
                String name = entry.getFileName();
                if (!".".equals(name) && !"..".equals(name)) {
    ...
    private static void copyToOutputStream(InputStream source, OutputStream os,
            Runnable onChunk) throws IOException {
