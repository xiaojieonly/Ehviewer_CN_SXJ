...
class OnlineTranslator:
    def __init__(self, api_type='google', api_key=None):
    ...
    def translate_text(self, text, source_lang, target_lang):
        ...
        try:
            if self.api_type == 'google':
                ...
                translated = self._translate_baidu(text, source_lang, target_lang)
    ...
    def _translate_baidu(self, text, source_lang, target_lang):
        """使用百度翻译API"""
        if not self.api_key:
            raise ValueError("百度翻译需要API密钥")
        ...
        # 计算签名
        appid = self.api_key.split('|')[0]
        secret_key = self.api_key.split('|')[1]
        sign_str = appid + text + params['salt'] + secret_key
        import hashlib
    ...
    def _translate_deepl(self, text, source_lang, target_lang):
        """使用DeepL翻译API"""
        if not self.api_key:
            raise ValueError("DeepL翻译需要API密钥")
...
class StringsTranslator:
    def __init__(self, base_file: str, translator: OnlineTranslator):
    ...
    def get_language_code(self, dir_name):
        """根据目录名获取语言代码"""
        language_codes = {
            'values-de': ('en', 'de'),  # 德语
            'values-en': ('en', 'en'),  # 英语
            'values-es': ('en', 'es'),  # 西班牙语
            'values-fr': ('en', 'fr'),  # 法语
            'values-ja': ('en', 'ja'),  # 日语
            'values-ko': ('en', 'ko'),  # 韩语
            'values-th': ('en', 'th'),  # 泰语
            'values-zh-rCN': ('en', 'zh'),  # 简体中文
            'values-zh-rHK': ('en', 'zh'),  # 繁体中文（香港）
            'values-zh-rTW': ('en', 'zh'),  # 繁体中文（台湾）
        }
        
        return language_codes.get(dir_name, ('en', 'en'))
    ...
    def translate_file(self, file_path: Path, dry_run=False):
        ...
        source_lang, target_lang = self.get_language_code(dir_name)
        
        print(f"翻译 {dir_name} ({source_lang} -> {target_lang})")
...
def main():
    parser = argparse.ArgumentParser(description='使用在线翻译API翻译strings.xml文件')
    parser.add_argument('--base', help='基础strings.xml文件路径（可选，会自动查找）')
    parser.add_argument('--api-type', default='google', 
                       choices=['google', 'baidu', 'deepl'],
                       help='翻译API类型')
    parser.add_argument('--api-key', help='API密钥（百度翻译需要appid|secret_key格式）')
    ...
    # 创建翻译器
    translator = OnlineTranslator(args.api_type, args.api_key)
