...
import com.hippo.anotherviewer.client.data.LargePreviewSet;
import com.hippo.anotherviewer.client.data.NewVersion;
...
import com.hippo.anotherviewer.client.exception.GalleryUnavailableException;
...
public class GalleryDetailParser {
     ...
     */
    public static GalleryDetail parse(String body) throws SiteException {
    ...
    private static void parseDetail(GalleryDetail gd, Document d, String body) throws ParseException {
        ...
        try {
            ...
            try {
                ...
                gd.category = SiteUtils.getCategory(ce.text());
            } catch (Throwable e) {
                ExceptionUtils.throwIfFatal(e);
                gd.category = SiteUtils.UNKNOWN;
            }
    ...
    private static String parseCoverStyle(String str) {
        ...
        if (matcher.find()) {
            return SiteUtils.handleThumbUrlResolution(matcher.group(3));
        } else {
            return "";
        }
    }
    ...
    private static GalleryTagGroup[] parseTagGroups(String body) throws SiteException {
    ...
    public static GalleryCommentList parseComments(Document document) {
        try {
            ...
            for (int i = 0, n = c1s.size(); i < n; i++) {
                ...
                if (null != comment && !SiteDB.inBlackList(comment.user)) {
                    list.add(comment);
                }
            }
    ...
    private static void find(Matcher m, LargePreviewSet largePreviewSet) {
        while (m.find()) {
            ...
            if (Settings.getFixThumbUrl()) {
                imageUrl = SiteUrl.getFixedPreviewThumbUrl(imageUrl);
            }
