...
class BackupServiceTest {
    ...
    fun setUp() {
        ...
        jdbc = JdbcTemplate(DriverManagerDataSource("jdbc:sqlite:${dataDir.resolve("ehviewer.db")}"))
        ...
        val config = EhCoreConfigProperties().apply {
    ...
    fun `export to restore round trip preserves db key and config`() {
        ...
        val checkJdbc = JdbcTemplate(DriverManagerDataSource("jdbc:sqlite:${dataDir.resolve("ehviewer.db")}"))
        ...
        assertTrue(Files.isRegularFile(dataDir.resolve("ehviewer.db.bak")))
    ...
    fun `tampered slice is rejected and original files untouched`() {
        ...
        val checkJdbc = JdbcTemplate(DriverManagerDataSource("jdbc:sqlite:${dataDir.resolve("ehviewer.db")}"))
        assertEquals("hello", checkJdbc.queryForObject("SELECT title FROM gallery WHERE id = 1", String::class.java))
        assertFalse(Files.exists(dataDir.resolve("ehviewer.db.bak")))
    }
